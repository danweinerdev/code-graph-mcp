...
- [ ] Flip plan README.md frontmatter `status: active` → `status: complete`. (Status flow per `shared/frontmatter-schema.md`: plan transitions `draft → approved → active → complete`. `/plan`'s approval step moves the folder `Plans/New/ → Plans/Ready/` and bumps the status to `approved`; `/planner:implement` moves it to `Plans/Active/` and flips to `active` when the first phase starts. This task's job is the final flip and the final move.)
- [ ] Move the plan folder to `Plans/Complete/CSharpJavaSupport` via `git mv` from its **current location**. Determine the source path at close-out time — if `/planner:implement` moved the plan to `Plans/Active/` (the expected path), use `git mv .plans/Plans/Active/CSharpJavaSupport .plans/Plans/Complete/CSharpJavaSupport`. If the plan never moved out of `Plans/Ready/` or `Plans/New/` for some reason, fall back to whichever directory currently holds the folder. Verify with `find .plans/Plans -name CSharpJavaSupport -type d` before running the `git mv`.
...
- [ ] Plan README.md status `active` → `complete`; folder moved from its current location (typically `Plans/Active/`) to `Plans/Complete/CSharpJavaSupport`
