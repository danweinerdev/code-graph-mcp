...
### Subtasks
- [ ] Add `limit: Option<u32>` and `offset: Option<u32>` to `GetFileSymbolsArgs`
- [ ] Add same pair to `GetCallersArgs`
- [ ] Add same pair to `GetCalleesArgs`
- [ ] Update each tool's `#[tool(description=...)]` macro text
- [ ] Confirm `JsonSchema` derive still works for all three structs
...
### Subtasks
- [ ] Add `limit` and `offset` parameters to `handlers::symbols::get_file_symbols`
- [ ] Preserve the empty-`file` arg check at the top (existing tool error)
- [ ] Read raw `file_symbols`; if empty, return existing `"no symbols found in file: <file>"` tool error — DO NOT wrap this in an envelope
- [ ] Apply the `top_level_only` filter, building `Vec<SymbolResult>` exactly as today
- [ ] After filtering, record `total = results.len() as u32`
- [ ] Stable sort by `symbol_id` ascending
- [ ] Slice with `iter().skip(offset).take(limit)`
- [ ] Wrap in `Page<SymbolResult>` and `tool_success_json` it
...
### Subtasks
- [ ] Add `limit` and `offset` parameters to `handlers::query::get_callers` and `get_callees`
- [ ] Run the existing BFS via `Graph::callers` / `Graph::callees`
- [ ] Sort the resulting `Vec<CallChain>` by `(depth, symbol_id)` ascending — explicit, do not rely on BFS visit order
- [ ] Record `total = vec.len() as u32`
- [ ] Slice with `iter().skip(offset).take(limit)`
- [ ] Wrap in `Page<CallChain>` and `tool_success_json` it
...
### Subtasks
- [ ] Update `ServerInner::get_file_symbols`, `get_callers`, `get_callees` to forward the new args
- [ ] Confirm no default-resolution leaks into the server layer

## 3.5: Update existing snapshots

### Subtasks
- [ ] `cargo test -p codegraph-tools` to regenerate
- [ ] `cargo insta review` — examine each of the five snapshots:
...
- [ ] For callers/callees, manually verify the new order is `(depth, symbol_id)` ascending — not BFS order
- [ ] Approve all five
...
### Subtasks
- [ ] Build a fixture with >100 symbols in a single file (or extend `engine.cpp`); write `response_get_file_symbols_paginated_offset` test with `offset=100 limit=50`
- [ ] Build (or find) a fixture with a high fan-in/fan-out symbol; write `response_get_callers_paginated_offset` and `response_get_callees_paginated_offset` tests
- [ ] Approve all three

## 3.7: Unit-test pagination invariants

### Subtasks
- [ ] For each tool: defaults, page union, total invariance, clamping, zero-limit, out-of-range offset
- [ ] `get_file_symbols`-specific: `empty_raw_set_returns_error` (asserts `tool_error` content, NOT envelope); `empty_post_filter_returns_empty_envelope` (top_level_only=true on a file with only methods → envelope with `total: 0`)
- [ ] `get_callers` / `get_callees`-specific: `pagination_orders_by_depth_then_symbol_id` (build fixture with multiple depths and verify order)

## 3.8: Update tools-list snapshots

### Subtasks
- [ ] Regenerate, review, approve for all three tools
- [ ] Verify the `inputSchema` descriptions match 3.1's text
...
- [ ] All three args structs gain `limit` + `offset` fields
- [ ] `get_file_symbols` preserves the empty-raw-set tool error; introduces empty-post-filter envelope path
- [ ] `get_callers` / `get_callees` sort by `(depth, symbol_id)` ascending — verified by a unit test
- [ ] Five existing response snapshots regenerated and approved
- [ ] Three new response snapshots added and approved
- [ ] Tools-list snapshots regenerated for all three tools
- [ ] Pagination unit tests pass for each tool, including tool-specific empty-set behavior
