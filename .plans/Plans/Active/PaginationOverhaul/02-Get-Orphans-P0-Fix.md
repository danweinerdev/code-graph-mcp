...
### Subtasks
- [ ] Add `limit: Option<u32>` with `#[serde(default)]` and a `#[schemars(description = "...")]` annotation
- [ ] Add `offset: Option<u32>` with same treatment
- [ ] Add `brief: Option<bool>` with same treatment, defaulting to true in the handler
- [ ] Update the `#[tool(description = "...")]` text on `ServerInner::get_orphans` to document `limit` (default 20, max 1000), `offset` (default 0), `brief` (default true) and the new envelope shape
...
### Subtasks
- [ ] Add a `limit: Option<u32>`, `offset: Option<u32>`, `brief: Option<bool>` parameter list to `handlers::structure::get_orphans`
- [ ] Resolve defaults: `limit = limit.filter(|&n| n != 0).unwrap_or(20).min(1000)`, `offset = offset.unwrap_or(0)`, `brief = brief.unwrap_or(true)`
- [ ] Read `Graph::orphans(parsed_kind)` into a `Vec<&Symbol>` (full match set)
- [ ] `let total = matches.len() as u32`
- [ ] Stable sort by `symbol_id` ascending (use the existing helper if there is one, else `sort_by_key(|s| s.id().clone())` or equivalent)
- [ ] Slice `[offset..offset+limit]` with bounds-safe slicing (use `iter().skip(offset).take(limit)` to avoid panics on out-of-range offsets)
- [ ] Map to `SymbolResult` with `symbol_to_result(s, brief)`
- [ ] Wrap in `Page<SymbolResult> { results, total, offset, limit }` and `tool_success_json` it
...
### Subtasks
- [ ] Update the `ServerInner::get_orphans` body to forward `args.limit`, `args.offset`, `args.brief` to the handler
- [ ] Confirm no default-resolution happens in `ServerInner` — the handler owns it (matches `search_symbols`'s pattern at the existing call site)
...
### Subtasks
- [ ] Run `cargo test -p codegraph-tools --test snapshot_responses response_get_orphans_default_callables`
- [ ] Run `cargo insta review` to inspect the regenerated `.snap.new` file
- [ ] Confirm the same set of symbol entries appears (no additions, no removals); order may differ (new sort by `symbol_id` ascending vs. prior insertion order)
...
### Subtasks
- [ ] Add `response_get_orphans_paginated_offset` test — fixture with at least 25 orphans (extend an existing fixture or build a new one), request `{offset: 20, limit: 20}`, snapshot the response
- [ ] Add `response_get_orphans_brief_false` test — same or smaller fixture, request `{brief: false}`, snapshot
- [ ] Add `response_get_orphans_offset_beyond_total` test — small fixture, request `{offset: 999}`, snapshot (assert `results: []`, `total > 0`)
- [ ] Approve all three via `cargo insta review`
...
### Subtasks
- [ ] Add `orphans_default_limit_is_20` test
- [ ] Add `orphans_page_1_and_page_2_cover_full_set` test on a 30-orphan fixture
- [ ] Add `orphans_total_is_full_match_count` test asserting `total` is consistent across pages
- [ ] Add `orphans_limit_clamps_at_1000` test
- [ ] Add `orphans_zero_limit_uses_default` test
- [ ] Add `orphans_offset_beyond_total_returns_empty` test
- [ ] Add `orphans_kind_filter_combined_with_pagination` test (e.g., kind=class with mixed orphan kinds)
...
### Subtasks
- [ ] Run `cargo test -p codegraph-tools --test snapshot_tools_list`
- [ ] Run `cargo insta review` for the `get_orphans` entry
- [ ] Confirm the `inputSchema` JSON shows the three new properties with the descriptions from 2.1
- [ ] Approve
...
- [ ] `cargo insta pending-snapshots` reports zero
- [ ] Quick manual check: `cargo run -p code-graph-mcp` against a small test repo, send a `get_orphans` request via MCP — confirm response shape is the envelope, not a flat array
...
- [ ] `GetOrphansArgs` has `limit`, `offset`, `brief` fields wired through to the handler
- [ ] Handler returns `Page<SymbolResult>` with correct defaults (20/0/true), clamping at 1000, and stable sort by `symbol_id`
- [ ] Existing `response_get_orphans_default_callables.snap` regenerated and approved (envelope shape; results unchanged byte-for-byte aside from wrapper)
- [ ] Three new response snapshots added and approved (page-2, brief=false, offset-beyond-total)
- [ ] Tools-list snapshot regenerated showing new args in `inputSchema`
- [ ] Seven new unit tests in `handlers/structure.rs` cover defaults, page union, total invariance, clamping, zero-limit, out-of-range offset, kind+pagination interaction
