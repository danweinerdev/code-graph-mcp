...
### Subtasks
- [x] Locate the entry point — likely `Graph::class_hierarchy` in `crates/codegraph-graph/src/queries.rs` calling into `algorithms.rs::build_hierarchy`
- [x] Thread a `max_nodes: u32` argument through the public API and the recursive helpers
- [x] Add a `&mut HashSet<String>` (or equivalent) to track unique names visited globally — separate from the per-path `on_path` set
- [x] Add the unique-name to the set *before* recursing; check `set.len() >= max_nodes` *before* recursing into each child
- [x] When the budget is exhausted, return early from the recursive call (do not add the child to `bases` / `derived`)
- [x] Track a `truncated: bool` flag — set whenever an early-return happens
- [x] Return `(HierarchyNode, u32 /* total_nodes_seen = visited.len() as u32 */, bool /* truncated */)` from the public function
...
### Subtasks
- [x] Add `class_hierarchy_max_nodes_truncates` test
- [x] Add `class_hierarchy_diamond_counts_unique_names` test (extends the existing diamond fixture)
- [x] Add a test confirming `max_nodes=u32::MAX` produces the same tree as today (regression guard for the algorithm change)
...
### Subtasks
- [x] Add `max_nodes: Option<u32>` with `#[serde(default)]` and a schemars description
- [x] Update the `#[tool(description=...)]` text to document `max_nodes` (default 250, max 1000), `truncated`, `total_nodes_seen`

## 4.4: Rewrite get_class_hierarchy handler

### Subtasks
- [x] Define a private `ClassHierarchyResponse { hierarchy: HierarchyNode, truncated: bool, max_nodes: u32, total_nodes_seen: u32 }` in `handlers/structure.rs`, derive `Serialize`
- [x] Resolve defaults: `max_nodes = max_nodes.filter(|&n| n != 0).unwrap_or(250).min(1000)`
- [x] Call `Graph::class_hierarchy(class, depth, max_nodes)`
- [x] Pack the result into `ClassHierarchyResponse` and `tool_success_json` it
...
### Subtasks
- [x] Forward `args.max_nodes` to the handler

## 4.6: Update existing class_hierarchy snapshots (×4)

### Subtasks
- [x] `cargo test -p codegraph-tools` to regenerate
- [x] `cargo insta review` for each of the four
- [x] Confirm `truncated: false` for all four fixtures (none of them approach 250 nodes)
- [x] Confirm `total_nodes_seen` equals the visible unique-name count of each tree
- [x] Approve

## 4.7: Add truncated-case snapshot

### Subtasks
- [x] Either build a >250-class hierarchy fixture, or pick an existing fixture and pass `max_nodes=5` to force truncation
- [x] Snapshot the response — assert `truncated: true`, `total_nodes_seen` equals the cap, partial tree is well-formed
- [x] Approve
...
### Subtasks
- [x] Regenerate, review, approve

## 4.9: Update CLAUDE.md and README.md

### Subtasks
- [x] Update `CLAUDE.md` "MCP Tools" section to reflect: `get_orphans` now has `limit`/`offset`/`brief`; `get_file_symbols` now has `limit`/`offset`; `get_callers`/`get_callees` now have `limit`/`offset`; `get_class_hierarchy` now has `max_nodes` and a wrapped response
- [x] Update top-level `README.md` if it documents tool args (check by grepping for tool names)
- [x] Confirm no stale references to flat-array responses
...
### Subtasks
- [x] `cargo test --workspace` (regenerates anything pending)
- [x] `cargo insta pending-snapshots` — must report zero
- [x] If non-zero, run `cargo insta review` and approve / reject case by case
- [x] `git status` shows the snapshot delta cleanly: ~22 snapshot files changed, no orphan `.snap.new` files
...
- [x] `cargo test --workspace` passes — full count matches or exceeds the baseline (683+ tests)
- [x] (Optional) Manual UE-style smoke test against a real C++ codebase — run `code-graph-mcp` via stdio, send `analyze_codebase`, then `get_orphans`, `get_class_hierarchy { class: UObject, depth: 2 }`, `get_callers { symbol: <high-fanin> }` — confirm responses are well-formed envelopes within MCP token ceiling
...
- [x] `Graph::class_hierarchy` accepts `max_nodes`; tracks unique names; returns `(root, total_nodes_seen, truncated)`
- [x] Diamond fixture passes with no semantic change; new diamond+max_nodes test confirms unique-name counting
- [x] `GetClassHierarchyArgs` has `max_nodes`; handler wraps response in `{hierarchy, truncated, max_nodes, total_nodes_seen}`
- [x] Four existing class_hierarchy snapshots regenerated; one new truncated snapshot added; tools-list snapshot regenerated
- [x] CLAUDE.md and README.md updated to reflect the new tool surface
- [x] Workspace-wide: zero pending snapshots, clippy-clean, fmt-clean, all tests passing
- [x] (Optional) Manual UE-scale smoke test confirms the original `get_orphans` token-limit failure is resolved
