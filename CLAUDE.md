...
| `code-graph-parse-test` | `crates/code-graph-parse-test` | Dev binaries: parser-corpus harness (`code-graph-parse-test`) and the repo-agnostic index/cache/verify benchmark (`code-graph-bench`). Not built into the MCP server. |
| `code-graph-lang-cpp` | `crates/code-graph-lang-cpp` | tree-sitter-cpp; scope-aware call resolution |
| `code-graph-lang-rust` | `crates/code-graph-lang-rust` | tree-sitter-rust; impl/trait, supertrait bounds, use-tree, `mod`-decl file edges, macro invocations |
...
- **MCP client tool-call timeout (Claude Code: `MCP_TOOL_TIMEOUT`) can kill `analyze_codebase` on large codebases without any server-side fault.** The Claude Code 2.x MCP client documents the env var as: *"Per-server tool-call timeout in milliseconds. Hard wall-clock limit per call; **progress notifications do not extend it**. Floored at 1000ms."* On a UE4-scale tree (or LLVM, measured: 72k files, 770k symbols, ~130-200s wall time) the analyze call can exceed the client default and surface to the agent as `"[Tool result missing due to internal error]"` — while the server runs to completion, writes the cache, and returns to idle. Verified empirically: a 136s analyze on LLVM tripped the client timeout even though the server's cache file landed cleanly and a follow-up `get_symbol_summary` against the same server returned `total: 13736` instantly. **No server-side mitigation rescues this** — the heartbeat language in the binary is explicit that progress notifications do not extend the deadline; rmcp's response went to a socket whose reader had already given up. **Operational requirements for clients indexing real codebases:** set `MCP_TOOL_TIMEOUT=900000` (15 min) in the environment that launches the MCP client, or pass `"timeout": 900000` in the per-server block of the client's MCP config (per Claude Code: *"Overrides the MCP_TOOL_TIMEOUT environment variable for this server"*). The 15-minute ceiling is sized for UE-style codebases on cold caches; LLVM-scale workloads on a warm SSD finish in 2-3 min but the headroom matters because the per-call timer is hard. **Diagnostic discriminator** for the `"tool result missing"` symptom: check the on-disk cache file's mtime vs the call timestamp and the server's RSS curve. If the cache file landed and RSS plateaued before the failure, the server completed and the client gave up — raise the timeout. If the server is still consuming CPU or the cache was not written, the operation is actually in flight or genuinely stuck — different problem.
...
| Group | Tools | Notes |
|---|---|---|
| Indexing | `analyze_codebase` | rkyv binary cache (`.code-graph-cache.db`) + mtime-based incremental re-index. Re-run with `force=true` to bypass cache (see Cache invalidation). |
| Symbol query | `get_file_symbols`, `search_symbols`, `get_symbol_detail`, `get_symbol_summary` | `get_file_symbols`: `top_level_only`/`brief` + `limit`/`offset` (default 100, max 1000); `count_only=true` returns total without records (< 1KB response); response capped at `[response].max_bytes` (default 100KB) — see Response shapes. `search_symbols`: `namespace` + `subtree` (optional dir prefix; routes through `PathTrie::iter_subtree`, O(subtree) not O(graph)) + `limit`/`offset` + `brief` (default true); `count_only=true` returns total without records (< 1KB response); returns `SearchSymbolsResponse` = flattened `Page<SymbolResult>` plus an optional `suggestions: string[]` (anchored-zero did-you-mean; absent from wire when empty) — see Response shapes. `get_symbol_summary`: `Page<SummaryRow>` (`{namespace, kind, count}`); empty namespace renders as `<global>`; `count_only=true` total = `(namespace,kind)` pair count — see Response shapes. |
| Call graph | `get_callers`, `get_callees` | `limit`/`offset` (default 100, max 1000); `min_confidence` (`"any"` default / `"resolved"`) drops Heuristic (multi-candidate) edges at BFS time — see Edge confidence; response capped at `[response].max_bytes` (default 100KB) — see Response shapes. |
...
| Structural | `detect_cycles`, `get_orphans`, `get_class_hierarchy`, `get_coupling` | `get_orphans`: `kind` + `subtree` (optional dir prefix; routes through `Graph::orphans_under` → `PathTrie::iter_subtree`) + `reliability` (`"all"` default / `"high"` drops virtual + macro-synth false-positive orphans) + `limit`/`offset`/`brief` (default 20, max 1000); `count_only=true` returns total without records (< 1KB response); response capped at `[response].max_bytes` (default 100KB) — see Response shapes. `detect_cycles`: `Page<Cycle>` (`{files, truncated, original_len?}`); paginated by COUNT not byte budget; `max_cycle_size` per-cycle file-list cap (default 50, max 500); `subtree` (optional dir prefix; **post-detection filter** — cycles are still detected over the whole graph but only those whose every file lies under `subtree` are reported, so a cycle that crosses the prefix boundary is silently dropped from the page) — see Response shapes and Known limitations. `get_class_hierarchy`: `depth` + `max_nodes` (default 250, max 1000); `HierarchyNode.ref` ref-stub for diamond duplicates — see Response shapes. `get_coupling`: `outgoing`/`incoming` → `Page<CouplingEntry>` (`{file, count}`); `both` → `CouplingBoth` (`{incoming, outgoing}`, two independent pages, no top-level `results`) — see Response shapes. |
| Viz | `generate_diagram` | call graph (`symbol=`) / file deps (`file=`) / inheritance (`class=`); `symbol=` mode takes `direction` (`callees`/`callers`/`both`) and `min_confidence` (`"any"` default / `"resolved"`) — see Edge confidence; `format=edges` rows are `{from, to, label, direction}` (`direction` serializes `"calls"`/`"called_by"`) — see Response shapes and Known limitations (lossy label collapse). |
...
- **`get_callers` / `get_callees`** → `Page<CallChain>`. Each `CallChain` = `{ symbol_id: string, file: string, line: u32, depth: u32 }`. **Field semantics (load-bearing across crates at depth ≥ 2):** `symbol_id` is the **definition site** (the callable being reported as a caller/callee), while `file`/`line` are the **call site** — the source file and line of the `Calls` edge that reached this hop (cloned straight from the `EdgeEntry`). At depth 1 `file` may or may not match the file segment of `symbol_id` (an inline call lives in the caller's file; a cross-crate call lives in the caller's file too). At depth ≥ 2 they routinely diverge across crates: a `CallChain` reporting `crates/foo/src/lib.rs:bar` with `file = "crates/baz/src/lib.rs"` means `bar` (defined in `foo`) was reached via an edge fired from `baz`. Clients wanting "where is this defined?" must split `symbol_id` on the rightmost `:` (not part of `::`), NOT read `file`. `depth` is the BFS distance from the requested symbol (1 = direct caller/callee). **Resolved-only filter (parity with `generate_diagram`):** hops whose target is not a resolved project symbol (same `is_resolved_node` BFS-expansion predicate used by `generate_diagram`) are dropped at BFS time, never enter the `visited` set, and never appear in the returned page. Bare-token unresolved callees/callers (e.g. `Ok`, `printf`, `to_string`, language-builtin macros/stdlib calls) are filtered out uniformly across all six languages — agents querying `get_callees` on a function whose only outgoing calls hit `unwrap`/`println!`/`fmt.Println` see an empty `Page<CallChain>`, NOT a page of bare tokens. **Non-callable soft-hint (success, not error):** calling `get_callers` / `get_callees` on a symbol whose `SymbolKind` is `Struct`, `Enum`, `Trait`, `Typedef`, or `Interface` returns a `CallToolResult` SUCCESS (`is_error: false`) whose body is a plain-text advisory naming the symbol basename + kind and pointing at the appropriate alternative tool — `get_class_hierarchy` or `get_symbol_detail` for structural kinds (`Struct`/`Enum`/`Trait`/`Interface`), `get_symbol_detail` for `Typedef`. **The advisory body is NOT the `Page<CallChain>` envelope** — it's a free-form string, parsing it as JSON will fail. Clients that pattern-match on the envelope must check `is_error` and try plain-text first before JSON-decoding. The branch is gated strictly on the non-callable kind set, so a *callable* symbol (`Function`/`Method`/`Class`) with zero resolved callers/callees still falls through to the empty `Page<CallChain>` envelope — preserving the "wrong tool" vs "wrong symbol" vs "no callers in scope" trichotomy (symbol-not-found → tool error with optional did-you-mean; non-callable kind → soft-hint success; callable with zero resolved hops → empty envelope). Standard `Page<T>` envelope (byte-budgeted; sort `(depth, symbol_id)` ascending so page 1 holds the closest hops).
...
- **`get_dependencies`** → `Page<DependencyEntry>`. Each `DependencyEntry` = `{ file: string, kind: string, line: u32 }`. `kind` is **always the string `"includes"`** for every language — Rust `mod` declarations, Python/Go/Java `import`, C# `using`, C++ `#include` all map to `EdgeKind::Includes` (the enum has exactly three variants: `Calls`, `Includes`, `Inherits`; there is no `"imports"` value). `line` is the source line the include/import directive was observed on. Only includes that resolve to an indexed source file appear: the indexer drops an `Includes` edge unless `resolve_include` returns a path AND the registry recognizes that file's language — ALL unresolved targets (system headers, external paths, `.ini`/`.cfg`/`.txt`, anything no plugin claims) are filtered at index time, never just `.ini`. **Rust specifically**: only intra-crate `mod foo;` declarations survive into `get_dependencies`/`detect_cycles`/`generate_diagram(file=)` — `use`/`extern crate` dotted tokens (`std::io`, `alloc`, `ark_core::reactor`) still emit at parse time but drop at resolve via `RustParser::resolve_include`'s pass-through (not a path → `None` → filtered), which is the intentional scope boundary, not a defect; see Rust "Supported" / "Limitations" sections below. Standard `Page<T>` envelope.
...
- **`generate_diagram`** — `format = "edges"` (default) returns a JSON array of `DiagramEdge` = `{ from, to, label, direction }`; `format = "mermaid"` returns Mermaid flowchart text. `from`/`to` are already-rendered display labels. For `symbol=` (call-graph) mode `label` is always the string `"calls"`. `direction` is a per-edge orientation tag that serializes as `"calls"` (outgoing — `from` calls `to`, discovered via forward adjacency) or `"called_by"` (incoming — `from` is an inbound caller of `to`, discovered via reverse adjacency); it is independent of the caller's `direction` request (`"callees"`/`"callers"`/`"both"`, symbol mode only). Empty edge list serializes as `[]`, never `null`. Edges whose endpoints don't resolve to a real symbol are dropped (no file-basename pseudo-node leak). The diagram edge set is deduped on the `(from, to)` pair: in `symbol=` mode that pair is the *rendered* label (lossy — see Known limitations); in `file=`/`class=` modes it is the full path / class-name identity (exact, no fidelity loss).

### Edge confidence (Calls / Inherits / Overrides only)

Every `EdgeEntry` carries a `Confidence` tag stamped by the resolver at index time. Two variants exist today (`Resolved`, `Heuristic`); the enum is `#[non_exhaustive]` so future per-language type-inference work can add finer-grained variants (`Template`, `DynamicDispatch`, …) without an MCP wire-format break.

- **`Resolved`** (default): the resolver was unambiguous about the target. Either the callee/include name had exactly one indexed candidate, OR the edge is declarative (Inherits / Overrides / Rust `mod`-resolved Includes / suffix-disambiguated includes — where the suffix rule `#include "foo/bar.h"` exactly matches `…/foo/bar.h` is structural, not a guess) and the parser had a known target at emission time.
- **`Heuristic`**: ≥ 2 indexed candidates shared the callee/basename and the resolver picked one via the scope rule (same file > same parent > same namespace > global) or first-of-N. The chosen target may or may not be the semantically correct one.

**`min_confidence` per-tool filter.** `get_callers`, `get_callees`, and `generate_diagram(symbol=…)` accept `min_confidence` with wire spelling `"any"` (default) or `"resolved"`. The filter applies at each BFS hop, so a depth-2 walk via a Heuristic intermediate is pruned in full — the intermediate's depth-1 row never enters the `visited` set, and its callees/callers at depth 2 never surface. The threshold parser is the shared `parse_min_confidence` helper in `crates/code-graph-tools/src/handlers/mod.rs`; an unknown spelling is a tool error.

**`get_dependencies` does NOT take `min_confidence`.** Include edges go through `IncludeEntry`, which does NOT carry confidence today — Include routing happens in `merge_file_graph`'s `EdgeKind::Includes` branch, which intentionally drops the confidence field on the floor because the suffix-disambiguation rule already collapses multi-candidate includes to a single Resolved-confidence result in 99% of cases. Adding confidence to `IncludeEntry` is a future-work item if Heuristic include resolution proves common enough to warrant a filter.

**`Confidence` does NOT extend `is_unreliable_orphan` (`get_orphans` reliability filter).** An orphan is a symbol with **zero** incoming Calls edges by definition; the Heuristic-vs-Resolved distinction applies only to edges that *exist*, so it has no leverage on the empty-inbound case. The orphan filter remains signature-driven (virtual + macro-synth) — see `get_orphans` description.

**Cache invalidation:** the `Confidence` field landed with `CACHE_VERSION` bump 5 → 6 (in-memory `EdgeEntry`) and was re-bumped 7 → 8 when the rkyv-packed `PackedEdge` schema gained its mirror field. See the `CACHE_VERSION` history above for the full sequence. Any pre-v8 cache fails the version check on `Graph::load` and the caller silently re-indexes.

## Configuration (`.code-graph.toml`)

Lives at the **project root** — defined as the nearest directory containing a `.code-graph.toml`, discovered by upward walk from the `analyze_codebase` invocation path. Walk semantics match cargo / git / rustfmt / editorconfig / npm: first match wins, no merging across multiple files, walk stops at filesystem root if nothing found (returning built-in defaults). Read once per `analyze_codebase` and cached for watch events. Sample at `.code-graph.toml.example` (repo root). All keys optional.

**Discovery + cache + scope split.** Three orthogonal concepts that all key off the project root in slightly different ways:
- **Config** is project-wide: the discovered `.code-graph.toml` rules apply to every invocation under the project root regardless of which subtree the user invoked from.
- **Cache** (`<project_root>/.code-graph-cache.db`) is project-wide: co-located with the config so scoped invocations from different subtrees share one cache and accumulate into it. The `.db` extension is the v7+ binary format (rkyv + 8-byte header). A leftover `.code-graph-cache.json` from a v5/v6 build is a separate inode and the v7+ loader treats it as "not present" without explicit recognition; safe to delete on upgrade.
- **Indexing scope** is invocation-local: `analyze_codebase(<subtree>)` parses only files under `<subtree>` even when the project root is upstream. The discovered config still applies (because rules are project-wide) but the file walk respects the invocation path. This is the lazy/scoped contract — you pay per scope you care about, not per full-project sweep.

**Nested `.code-graph.toml` shadows its ancestor.** A `.code-graph.toml` inside a subdirectory of a configured project marks that subdirectory as its own project: discovery from inside the inner subtree stops at the inner toml and the cache lands there. No config merging. Useful for monorepos where one subproject needs distinct macro/extension rules.

**No config found anywhere** → built-in defaults + a warning surfaced through `AnalyzeResult.warnings` explicitly calling out that engine-style classes (`class CORE_API Foo`) will not extract. Configuration discovery is silent only when the user has placed a toml at the project root *or* when the user has no config and the codebase doesn't need one (the no-config warning is informational, not an error).
...
- **Cache co-location at the project root.** `cache_path` resolves to `<project_root>/.code-graph-cache.db` (where `project_root` is the discovered config dir, or the invocation dir if no config found). Two invocations at different subtrees of the same project share one cache and accumulate into it; the cache survives across scoped invocations rather than being overwritten. A pre-fix cache file at an invocation subdir is reported as "orphan cache" in `AnalyzeResult.warnings` so the user can reclaim disk — the new run does NOT touch the orphan, and its own cache lands at the discovered project root.
- **Merge-not-clobber semantics for scoped invocations.** `analyze_codebase(<subtree>)` without `force=true` loads the existing project cache, evicts in-scope files that no longer exist on disk, parses files within the invocation scope, and merges them into the cached project graph. Files outside the invocation scope are preserved untouched — the lazy/scoped contract. Subsequent scoped invocations at sibling subtrees accumulate into the same project graph, so the user gets a project-wide graph by indexing the subtrees they care about over time.
- **Scoped `force=true` is scope-limited cache invalidation.** `force=true` at the project root clobbers and rebuilds the whole project. `force=true` at a subdir drops only the cache entries inside that subdir before re-indexing — entries from sibling subtrees (added by prior scoped invocations) survive. The mental model: `force=true` invalidates exactly the subtree the user invoked at, never more.
- **Cross-scope edge resolution is asymmetric by design.** Fresh files' edges resolve against the union of cached symbols (from prior scoped invocations) and freshly-parsed symbols — so a fresh-to-cached edge resolves correctly. The reverse (a cached edge from a prior invocation pointing into a newly-indexed subtree) does NOT spontaneously resolve, because cached edges are not re-resolved on subsequent invocations. The user can `force=true` at the originating subtree to re-parse and re-resolve. This bounds the resolve phase cost to the size of the freshly-parsed set rather than the size of the project, at the price of "edges from older subtrees may not pick up symbols added by newer subtrees until the older subtree is forced." See `crates/code-graph-tools/src/indexer.rs::resolve_edges_with_indexes` doc-comment for the implementation contract.
- **Opportunistic out-of-scope hygiene sweep.** Each `analyze_codebase` call checks `Graph::last_sweep_at`; if at least `SWEEP_INTERVAL_NANOS` (default: 24h, see `crates/code-graph-graph/src/persist/mod.rs`) has elapsed since the last sweep, the handler runs `Graph::sweep_missing_out_of_scope(invocation_path)` to stat every cached file OUTSIDE the invocation scope and drop the ones no longer on disk. Keeps the project cache from accumulating ghost entries for deleted files in subtrees the user hasn't touched recently, without paying full-revalidation cost on every invocation. The sweep timestamp is persisted in the cache (`last_sweep_at` field of `PackedCacheV6`) so the cadence survives process restarts.
- **`CACHE_VERSION` history** (single source of truth: `crates/code-graph-graph/src/persist/packed.rs::CACHE_VERSION`, currently `8`). Every bump triggers the documented contract: cache fails the version check on `Graph::load` → returns `Ok(false)` → caller **silently re-indexes**, no `force=true` required and no transparent migration attempted.
  - v3: include entries gained `{path, line}` records (was bare path list) so `get_dependencies` could report source-line per import. The unresolved-include filter at index time (drop any `Includes` edge whose `resolve_include` returns `None` or whose target isn't a registered-extension source file) means re-indexed graphs are clean of `.ini`/`.txt`/system-header noise going forward, without `force=true`.
  - v4: Rust symbol shape — trait default and abstract methods now classify as `Method`/parent=trait (was: `Function` or absent), and `Symbol.namespace` is rewritten at index time to the crate-qualified module path (`crate_name::a::b`) by `RustParser::post_index`.
  - v5: added `last_sweep_at: u64` (UNIX_EPOCH ns) for the out-of-scope hygiene cadence.
  - v6: added `confidence: Confidence` to every `Edge` (parser-side) and `EdgeEntry` (cache-side) tagging single-candidate-match (`Resolved`) vs scope-heuristic-pick (`Heuristic`). Powers the `min_confidence` filter on `get_callers`/`get_callees`/`generate_diagram(symbol=…)` — see "Edge confidence" under Response shapes.
  - v7: format flip — interned JSON cache replaced by a rkyv binary archive prepended by an 8-byte header (`ENDIAN_PROBE: u32 native` + `CACHE_VERSION: u32 native`). The 4-byte endian probe (`0x01020304`) catches cross-endian mmap attempts (an Apple-ARM-written cache on a volume mmap'd by an Intel-Linux container) and routes them to silent re-index. The file extension changed from `.code-graph-cache.json` to `.code-graph-cache.db` — a separate-inode discriminator so the loader treats a leftover JSON cache as "not present" without explicit format-sniffing.
  - v8: extended `PackedEdge` (the rkyv-archived edge record) with its own `confidence` field so the v6 in-memory `Confidence` round-trips through the packed schema. Without the bump, two v7 binaries — pre-fixup and post-fixup — would write archives with different `PackedEdge` byte layouts and bytecheck would catch the size mismatch without surfacing a version-mismatch diagnostic.
...
- **Crate-qualified namespaces.** `Symbol.namespace` is rewritten at index time (in `RustParser::post_index`) to the canonical crate-qualified module path: `crate_name::a::b` for `src/a/b.rs` of a crate whose `[package].name` is `crate_name` (`-` in the crate name → `_`); `crate_name` alone for `lib.rs` / `main.rs`; `crate_name::a` for `src/a/mod.rs`. `#[path = "x.rs"]` overrides are NOT applied to **namespace assignment** (the `CrateModuleModel::with_path_overrides` seam exists but `post_index`'s namespace pass does not yet feed it `#[path]` attributes from source — symbols in files reached via `#[path]` still get namespaces derived from their on-disk location). `#[path]` IS honored by `mod`-edge resolution (see next bullet) — the two are intentionally decoupled. Inline `mod a { mod b { … } }` composes onto the crate prefix → `crate_name::a::b`. **No-Cargo.toml fallback**: a `.rs` file with no discoverable Cargo.toml gets an inline-`mod`-only namespace (same fallback as files outside any Cargo crate — `a::b` for nested inline mods, empty for a top-level item), and an empty namespace still renders as `<global>` via `get_symbol_summary`. `mod_item`s themselves do NOT produce Symbol records (namespace anchors only).
- **`mod`-declaration file edges.** External `mod foo;` declarations emit one file-level `Includes` edge per declaration (`from = declaring file`, `to = resolved child file`, `line = mod_item line`). Resolution is performed in `RustParser::post_index` using Rust's module-file rules with `#[path = "x.rs"]` → sibling `dir/foo.rs` → `dir/foo/mod.rs` priority (first indexed candidate wins; `#[path]` is authoritative — no fall-through to sibling/`mod.rs` if the override target is unindexed). Inline `mod foo { … }` bodies live in the same file and emit no edge (the self-edge is suppressed at parse time). Edges whose resolved candidate is not in the `FileIndex` are dropped here (no bare-modname `to` ever reaches `resolve_all_edges`). This is what makes `get_dependencies`, `detect_cycles`, and `generate_diagram(file=)` functional for a real Rust crate — the intra-crate module tree.
- **Supertrait `Inherits` edges.** `trait Sub: Super { … }` emits one `Inherits` edge per nameable supertrait bound (`from = Sub`, `to = Super`); `trait S: A + B + 'a + ?Sized` emits two edges (`S → A`, `S → B`) with lifetime / `?Sized` / unhandled-bound kinds filtered. Driven by a dedicated `SUPERTRAIT_QUERY` and a second independent loop in `extract_inheritance`, kept separate from the `impl`-loop to keep its accumulate-then-emit shape (one match → one edge) intact while the supertrait loop emits per-bound. `EdgeKind::Inherits` is reused (no enum change).
- All `use`-tree forms (simple, scoped, grouped, nested grouped, wildcard, aliased records the path, `self`-in-list, `extern crate`) are still extracted into provisional `Includes` edges, but they **drop at resolve time** via `RustParser::resolve_include` (a dotted token like `"std::io"` or `"alloc"` is not an absolute path, returns `None`, the surrounding loop drops the edge). This is an intentional scope boundary, not a defect — `use`/`extern crate` paths are intentionally not resolved to files (no crate-aware `use`-path resolver; `mod`-decl resolution is the model). Result: zero `use`/`extern crate` survivors in `get_dependencies`/`detect_cycles`/file diagrams; `mod`-decl edges are the sole intra-crate file-dep signal.
...
- `package_clause` → `Symbol.namespace`, then optionally **rewritten at index time** to the module-qualified path (`module_path::a::b`) by `GoParser::post_index` if a `go.mod` is discoverable upward from the file. The Go Module Model walks ancestor dirs at index time, parses `module <name>` from each `go.mod`, and prepends the matching module's `name + relative_path_segments` to the package clause's namespace. Files outside any discoverable module fall back to the bare package name. `mod_item`s themselves do NOT produce Symbol records.
...
4. **`go.mod` consulted for namespace derivation only; vendor NOT consulted.** `GoParser::post_index` reads `go.mod` to rewrite per-file `Symbol.namespace` to the module-qualified form (see Supported above). Import-path resolution to file edges is unchanged — import paths are still recorded verbatim in `Includes.to` and the default basename match against FileIndex is correctly a no-op for module paths. Net effect: `get_symbol_summary` and `search_symbols` operate against fully-qualified namespaces (good); `get_dependencies` still does NOT resolve cross-module imports to indexed Go files (unchanged limitation).
...
History: this lens has caught real agent-misleading bugs — e.g. `get_callers`/`get_callees` descriptions advising "raise via offset" when `offset` is a skip-count, and over-confident "typical depth=1/2 walks" guidance on `get_class_hierarchy`.
...
Checklist:
- **Framing contradictions across sibling sections.** A feature documented in two places (e.g. "Supported Patterns" + "Known Limitations") should convey consistent signals — a "supported" claim and a "known limitation" describing the same case must agree on what works.
