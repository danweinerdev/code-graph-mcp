...
- **Claude Code `MCP_TOOL_TIMEOUT` can kill `analyze_codebase` on large codebases.** Hard wall-clock per call; progress notifications do NOT extend it (floor 1000ms). UE4/LLVM-scale trees (72k files, 770k symbols, ~130-200s wall time) can exceed the client default and surface to the agent as `"[Tool result missing due to internal error]"` while the server runs to completion, writes the cache, and returns to idle. **No server-side mitigation rescues this.** Set `MCP_TOOL_TIMEOUT=900000` (15 min) in the environment or `"timeout": 900000` in the per-server MCP config block. **Diagnostic discriminator:** if cache file mtime lands and RSS plateaus before the failure, the server completed and the client gave up — raise the timeout. If RSS is still climbing or no cache wrote, the operation is genuinely in flight or stuck — different problem. **Structural workaround:** clients hitting `MCP_TOOL_TIMEOUT` on long analyses should prefer `analyze_codebase_async` + poll `get_status` — every individual tool call is sub-second, so the per-call timer never fires. The result lives on `get_status.analyze_job.result` once `status` transitions to `"completed"`.
- **Sync `analyze_codebase` now waits for in-flight watch reindex.** Pre-AnalyzeCodebaseAsync, sync `analyze_codebase` called `index_lock.try_lock` and returned `"indexing already in progress"` when a `watch_start`-driven reindex happened to be mid-flight — misleading wording, since no other *analyze* was actually running. Under the slot model the slot is the single-flight gate for analyses (Design Decision 1); `index_lock` only serializes worker vs. watch reindex. Sync now writes the slot, then `index_lock.lock().await`s — a watch reindex in flight delays the analyze's start by a single file's parse+merge (typically milliseconds) instead of failing the call. The `"indexing already in progress"` error wording is preserved but now only fires on actual analyze-vs-analyze contention (Design Decision 9).
...
| Group | Tools | Notes |
|---|---|---|
| Indexing | `analyze_codebase`, `analyze_codebase_async` | `analyze_codebase`: rkyv binary cache + mtime-based incremental re-index. `force=true` bypasses cache. Under watch contention, sync `analyze_codebase` now awaits the watch reindex (typically ms) rather than returning an error — the slot is the single-flight gate (Design Decision 9). `analyze_codebase_async`: same args + same indexing pipeline, but kicks off a detached worker and returns sub-second with `{ job_id, status: "running", started_at, existing, note }`. Agents poll `get_status.analyze_job` for progress and `analyze_job.result` / `.error` on terminal. Duplicate kickoff while a job is `Running` returns the in-flight `job_id` with `existing: true` (args ignored). |
...
| Status | `get_status` | Diagnostic snapshot — binary SHA, config path, indexed root, graph stats, last analyze timestamp + force flag. When an analyze has ever run, also carries `analyze_job` (current slot) and `analyze_job_previous_terminal` (one-rotation grace window) — see `AnalyzeJobView` under Response shapes. Both fields serialize as explicit `null` when absent so clients can distinguish "no analyze ever" from "missing field on an old server". |
...
- **`AnalyzeJobView`** (`get_status.analyze_job` / `get_status.analyze_job_previous_terminal`) → `{ job_id, status, path, force, started_at, finished_at, progress, progress_total, progress_message, error, result }` (11 fields). `status` is a lowercase string discriminator — `"running"` | `"completed"` | `"failed"` (NOT the Rust enum tag). `job_id` is a 20-char zero-padded decimal nanosecond timestamp (Decision 6 — unique-by-construction under single-flight). `started_at` is always RFC3339 UTC; `finished_at` is RFC3339 UTC when `status != "running"`, `null` otherwise. `progress` is files processed in the current phase — monotonic non-decreasing **within a phase**, resets at each phase boundary (parse → resolve → merge); phase identity rides on `progress_message`, so consumers asserting monotonicity must scope to a single phase. `progress_total` is `0` during discovery, set once the indexer knows the file universe; `progress_message` is the worker's most recent phase label (e.g. `"Parsing: <path>"`, `"Resolving: <path>"`). **Discriminator exclusivity:** `error` is `Some(String)` ONLY when `status == "failed"`; `result` is `Some(AnalyzeResult)` ONLY when `status == "completed"`; both are `null` while `Running`, and they never co-occur. `result` is byte-identical in shape to `analyze_codebase`'s success body (`{ files, symbols, edges, root_path, warnings }`) — a single deserializer covers both. **Slot rotation (`analyze_job` vs. `analyze_job_previous_terminal`):** `analyze_job` is the slot's `current` — the in-flight job, or the most-recent terminal until a new kickoff rotates it out. `analyze_job_previous_terminal` is the slot's `previous_terminal` — the prior job preserved across exactly ONE additional kickoff (Design Decision 4 grace window). Two analyses in a row without reading the first lose the oldest. **Null-serialization invariant:** both `analyze_job` and `analyze_job_previous_terminal` emit explicit `null` (no `skip_serializing_if`) when absent — matches the `index_force_built` precedent in the same envelope, so clients can distinguish "no analyze ever" from "missing field on an old server" by field presence.
