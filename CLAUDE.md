...
- **Crate-qualified namespaces.** `Symbol.namespace` is rewritten at index time (in `RustParser::post_index`) to the canonical crate-qualified module path: `crate_name::a::b` for `src/a/b.rs` of a crate whose `[package].name` is `crate_name` (`-` in the crate name → `_`); `crate_name` alone for `lib.rs` / `main.rs`; `crate_name::a` for `src/a/mod.rs`; `#[path = "x.rs"]` overrides apply. Inline `mod a { mod b { … } }` composes onto the crate prefix → `crate_name::a::b`. **No-Cargo.toml fallback**: a `.rs` file with no discoverable Cargo.toml gets an inline-`mod`-only namespace (today's pre-Phase-1 behavior — `a::b` for nested inline mods, empty for a top-level item), and an empty namespace still renders as `<global>` via `get_symbol_summary`. `mod_item`s themselves do NOT produce Symbol records (namespace anchors only).
