...
- **Watch-mode path re-contamination on Windows.** `notify-debouncer-full` may deliver `\\?\`-prefixed event paths during reindex (`crates/code-graph-tools/src/handlers/watch.rs`), re-contaminating a graph that was clean post-canonicalization. Until fixed, Windows watch-mode on UE-style codebases may see `\\?\` strings creep back into symbol IDs after the first watched edit.
