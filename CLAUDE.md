...
**Cache invalidation:** the `Confidence` field landed with `CACHE_VERSION` bump 5 → 6. A v5 cache fails the version check on `Graph::load` and the caller silently re-indexes. The `#[serde(default)]` on the field is defense-in-depth for hand-written fixtures; production caches never hit the default.
...
- **Cache** (`<project_root>/.code-graph-cache.json`) is project-wide: co-located with the config so scoped invocations from different subtrees share one cache and accumulate into it.
...
- **Opportunistic out-of-scope hygiene sweep.** Each `analyze_codebase` call checks `Graph::last_sweep_at`; if at least `SWEEP_INTERVAL_NANOS` (default: 24h, see `crates/code-graph-graph/src/persist.rs`) has elapsed since the last sweep, the handler runs `Graph::sweep_missing_out_of_scope(invocation_path)` to stat every cached file OUTSIDE the invocation scope and drop the ones no longer on disk. Keeps the project cache from accumulating ghost entries for deleted files in subtrees the user hasn't touched recently, without paying full-revalidation cost on every invocation. The sweep timestamp is persisted in the cache (`GraphCache.last_sweep_at`) so the cadence survives process restarts.
...
- **`CACHE_VERSION` bump (include entries now carry source lines).** Include entries in the cache went from a bare path list to per-include `{path, line}` records so `get_dependencies` can report the source line of each include/import. The shape change is **not** backward-compatible: a v1 (Go-written), v2, or v3 cache fails the version check on `Graph::load`, which surfaces as `Ok(false)` so the caller **silently re-indexes — no `force=true` required and no transparent migration is attempted** (distinct from the path-string migration above, which is in-place). The unresolved-include filter (drop any `Includes` edge that doesn't resolve to an indexed-language source file — see `resolve_all_edges` in `crates/code-graph-tools/src/indexer.rs`) lives at the indexer/edge-resolution layer, so every re-indexed graph is clean of `.ini`/`.txt`/system-header/unresolvable include edges going forward without `force=true`.
- **`CACHE_VERSION` bump (Rust symbol shape: trait methods + crate-qualified namespaces).** v4 captured Rust symbol-shape changes that change the cached records: trait default methods and abstract trait-method signatures are now `Method`/parent=trait (was: `Function` or absent), and Rust `Symbol.namespace` is rewritten at index time to the crate-qualified module path (`crate_name::a::b`). Same invalidation contract as the v2→v3 bump above: an older cache fails the version check on `Graph::load`, which returns `Ok(false)` → caller **silently re-indexes — no `force=true` required and no transparent migration is attempted**.
- **`CACHE_VERSION` bump (out-of-scope sweep timestamp).** v5 adds a `last_sweep_at: u64` field (nanoseconds since UNIX_EPOCH) tracking when the opportunistic out-of-scope hygiene sweep last ran, so the cadence survives process restarts. The persisted JSON shape gains one optional field; older caches fail the version check on `Graph::load` and surface as `Ok(false)` → caller silently re-indexes (same invalidation contract as every prior bump). The first analyze against a fresh cache observes `last_sweep_at == 0` and runs the sweep immediately as part of its bookkeeping.
- **`CACHE_VERSION` bump (per-edge resolver confidence).** `CACHE_VERSION` is `6` (`crates/code-graph-graph/src/persist.rs`). The bump adds a `confidence: Confidence` field to every `Edge` (parser-side) and `EdgeEntry` (cache-side) tagging whether the resolver had a sole-candidate match (`Resolved`) or picked among ≥ 2 same-name candidates via the scope heuristic (`Heuristic`). Powers the `min_confidence` filter on `get_callers`/`get_callees`/`generate_diagram(symbol=…)` — see "Edge confidence" under Response shapes. Same invalidation contract as every prior bump: a v5 cache fails the version check on `Graph::load` and the caller silently re-indexes; the `#[serde(default)]` on the field is defense-in-depth for hand-written fixtures.
