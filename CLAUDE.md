...
- **`CACHE_VERSION` bump (out-of-scope sweep timestamp).** `CACHE_VERSION` is `5` (`crates/code-graph-graph/src/persist.rs`). The bump adds a `last_sweep_at: u64` field (nanoseconds since UNIX_EPOCH) tracking when the opportunistic out-of-scope hygiene sweep last ran, so the cadence survives process restarts. The persisted JSON shape gains one optional field; older caches fail the version check on `Graph::load` and surface as `Ok(false)` → caller silently re-indexes (same invalidation contract as every prior bump). The first analyze against a fresh v5 cache observes `last_sweep_at == 0` and runs the sweep immediately as part of its bookkeeping.
