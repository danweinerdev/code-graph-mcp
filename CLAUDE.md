...
- **`CACHE_VERSION` bump (Rust symbol shape: trait methods + crate-qualified namespaces).** `CACHE_VERSION` is `4` (`crates/code-graph-graph/src/persist.rs`). The bump captures Rust symbol-shape changes that change the cached records: trait default methods and abstract trait-method signatures are now `Method`/parent=trait (was: `Function` or absent), and Rust `Symbol.namespace` is rewritten at index time to the crate-qualified module path (`crate_name::a::b`). Same invalidation contract as the v2→v3 bump above: a v3 (or older v2) cache fails the version check on `Graph::load`, which returns `Ok(false)` → caller **silently re-indexes — no `force=true` required and no transparent migration is attempted**.
