//! Synthetic regression fixture for `PaginatedResponseSizeSafety` Phase 5.
//!
...
//! ceiling. Phases 1-3 of the plan added the byte budget, the pagination
//! envelope, the `count_only` short-circuit, and the `SymbolResult.file`
//! drop. This fixture is the substrate the Phase 5 acceptance test runs
//! against to GUARANTEE that those guard rails actually bite at the default
//! `[response].max_bytes = 102400` budget on `limit = 1000`.
//!
//! # Why N = 1500
//!
//! After Phase 3.4 dropped `SymbolResult.file`, a brief-mode record for a
...
//! `byte_budget_acceptance.rs`, see Phase 5.2):
//!
...
//!   (Phase 5.2): the second page picks up where the first left off.
//!
//! Pre-Phase-3 records were ~3x larger (the dropped `file` field +
//! signature padding), so the same byte-budget assertion was trivially
//! satisfied at N = 1000. The post-Phase-3 record shrinkage is exactly
//! the reason `N >= 1500` is now the load-bearing floor — a fixture
//! sized at the pre-Phase-3 numbers would silently pass without
//! exercising the truncation path, which is the false-green failure mode
//! the plan reviewer warned about.
//!
//! # Determinism
//!
...
/// Cardinality used by the Phase 5 acceptance test. See module-level docs
/// for the rationale tying this number to the `max_bytes = 102_400` /
...
pub fn generate_large_orphan_source(n: usize) -> String {
    ...
    source.push_str("// Anti-regression fixture for PaginatedResponseSizeSafety Phase 5.\n");
...
mod tests {
    ...
    fn generate_is_deterministic() {
        ...
        // byte-identical output. Phase 5's acceptance test depends on this
        // so the continuation-offset arithmetic stays reproducible.
    ...
    fn generate_zero_pads_to_four_digits() {
        ...
        // continuation-offset reasoning in Phase 5.2's paging loop.
