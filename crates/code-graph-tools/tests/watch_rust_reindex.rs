...
//!
//! Mirrors the structure of `watch_dangling_edges.rs` but
...
//!      dangling edges after a re-parse) is exercised by Rust changes the
...
/// CRITICAL: a watch-driven reindex of a `.rs`
...
///      — it must hold for the Rust plugin the same way
///      it does for C++).
#[tokio::test]
...
/// Lifecycle test: `watch_start` against a Rust temp project
